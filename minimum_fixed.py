...
def optimize_antenna_placement(n):
    ...
    # 目的関数の設定
    objective = P_min * gp.quicksum(X[i] for i in range(k**2))
    ...
    for i in range(k**2):
        for j in range(n):
            ...
            model.addConstr(W[i, j] == (distance <= r_min**2))
...
# 結果をエクセルファイルに書き出す
result_df.to_excel("optimization_result_minimum.xlsx", index=False)
